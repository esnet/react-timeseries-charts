...
export class Styler {
        ...
        if (_.isArray(columns)) {
            columns.forEach(column => {
                if (_.isString(column)) {
                    ...
                    this._columnStyles[key] = style;
                }
            });
        }
        this._columnNames = _.map(columns,
            c => _.isString(c) ? c : c.key);
    ...
    colorLookup(columnCount) {
        const maxSchemeSize = _.max(_.keys(colorbrewer[this._colorScheme]));
        const colorLookupSize = columnCount > maxSchemeSize ? maxSchemeSize : columnCount;
        return this._colorScheme ? colorbrewer[this._colorScheme][colorLookupSize] : [];
    }
    ...
    legendStyle(column, type) {
        ...
        const {color, width = 1, dashed = false} = this._columnStyles[columnName];
    ...
    axisStyle(column) {
        ...
        const i = _.indexOf(this._columnNames, column);
        const columnName = this._columnNames[i];
        const {color} = this._columnStyles[columnName];
        const c = color ? color : colorLookup[i % colorLookup.length];
        return {
            labelColor: c
        };
...
export default function style(columns, scheme) {
