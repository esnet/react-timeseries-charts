...
export default class EventHandler extends React.Component {
    ...
    handleScrollWheel(e) {
        if (!this.props.enablePanZoom && !this.props.dragToZoom) {
            return;
        }
    ...
    handleMouseUp(e) {
        if (!this.props.onMouseClick && !this.props.enablePanZoom && !this.props.dragToZoom) {
            return;
        }
    ...
    handleMouseMove(e) {
        ...
        if (this.state.isPanning) {
            ...
            const trackerPosition = offsetxy[0];
            const time = this.props.scale.invert(trackerPosition);
            if (this.props.onMouseMove) {
                this.props.onMouseMove(time, offsetxy[1]);
            }
        }
    }
