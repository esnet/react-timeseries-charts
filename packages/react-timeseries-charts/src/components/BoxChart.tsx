...
function getAggregatedSeries(
    ...
    column: string,
    aggregation: AggregationSpec = defaultAggregation): TimeSeries<Index> {
 ...
 */
export default class BoxChart extends React.Component<BoxChartProps> {
        ...
        if (seriesChanged) {
            if (nextProps.series.size() > 0) {
                if (nextProps.series.atFirst().keyType() === "time") {
                    ...
                    this.series = getSeries(nextProps.series as TimeSeries<Index>, nextProps.column);
                }
            }
        }
        return (
     ...
     */
    providedStyleArray(column: string): ChannelStyle {
        let style: ChannelStyle = defaultStyle;
        if (this.props.style) {
