...
exports.default = _react2.default.createClass({
    ...
    createScale: function createScale(yaxis, type, min, max, y0, y1) {
        ...
        if (_underscore2.default.isUndefined(min) || min !== min || _underscore2.default.isUndefined(max) || max !== max) {
            ...
            scale = _d2.default.scale.linear().domain([min, max]).range([y0, y1]).nice();
        } else if (type === "log") {
            var base = yaxis.props.logBase || 10;
            scale = _d2.default.scale.log().base(base).domain([min, max]).range([y0, y1]);
        } else if (type === "power") {
            var power = yaxis.props.powerExponent || 2;
            scale = _d2.default.scale.pow().exponent(power).domain([min, max]).range([y0, y1]);
        }
        return scale;
    },
