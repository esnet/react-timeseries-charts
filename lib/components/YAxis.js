...
var YAxis = function (_React$Component) {
    ...
    _createClass(YAxis, [{
        ...
        value: function postSelect(style, hideAxisLine, height) {
            ...
            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".tick").select("text").style(valueStyle);

            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".tick").select("line").style(tickStyle);

            (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").selectAll(".domain").remove();

            if (!hideAxisLine) {
                (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)).select("g").append("line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", height).style(axisStyle);
            }
        }
    }, {
        key: "generator",
        ...
        value: function renderAxis(align, scale, width, height, showGrid, chartExtent, hideAxisLine, absolute, type, fmt, label, tickCount, min, max) {
            ...
            console.log((0, _d3Selection.select)(_reactDom2.default.findDOMNode(this)));

            // Add the new axis
            this.axis = (0, _d3Selection.select)(_reactDom2.default.findDOMNode(this));

            this.axis.append("g").attr("transform", "translate(" + x + ",0)").attr("class", "yaxis").call(axisGenerator.tickSize(tickSize)).style(valueStyle);

            this.axis.append("text").text(label || this.props.label).attr("transform", "rotate(-90)").attr("class", "yaxislabel").attr("y", labelOffset).attr("dy", ".71em").attr("text-anchor", "end").style(labelStyle);
